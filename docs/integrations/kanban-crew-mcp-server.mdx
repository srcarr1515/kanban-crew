---
title: "Kanban Crew MCP Server"
description: "Configure the Kanban Crew MCP server"
---

Kanban Crew exposes a local MCP (Model Context Protocol) server, allowing you to manage organisations, projects, issues, workspaces, and repositories from external MCP clients like Claude Desktop, Raycast, or even coding agents running within Kanban Crew itself.

<Note>
This page covers connecting **external MCP clients** to Kanban Crew's MCP server. For configuring MCP servers **within** Kanban Crew for your coding agents, see the [MCP Server Configuration](/integrations/mcp-server-configuration) guide.
</Note>

<Info>
Kanban Crew's MCP server is **local-only** - it runs on your computer and can only be accessed by applications installed locally. It cannot be accessed via publicly accessible URLs.
</Info>
...
This works if you're adding the Kanban Crew MCP server to any [supported coding agent](/supported-coding-agents) **within** Kanban Crew.

1. In Kanban Crew Settings, navigate to the "MCP Servers" page
2. In the "Popular servers" section, click on the Kanban Crew card
3. Click the `Save Settings` button

<Frame>
<img src="/images/vk-mcp-server-config.png" alt="MCP Servers configuration page showing how to add Kanban Crew MCP server" />
</Frame>
...
```json
{
  "mcpServers": {
    "kanban_crew": {
      "command": "npx",
      "args": ["-y", "kanban-crew@latest", "--mcp"]
    }
  }
}
```

`--mcp` launches the local MCP stdio server. Any additional arguments after `--mcp` are passed through to the `kanban-crew-mcp` binary.

## Available MCP Tools

The Kanban Crew MCP server provides tools for managing organisations, projects, issues, workspaces, and task execution.
...
3. **Automatic Issue Generation**: Your MCP client will use the Kanban Crew MCP server to automatically create structured issues in your project
...
Your MCP client will use the `create_issue` tool to break this down into individual issues and add them to your Kanban Crew project automatically.
...
### Internal Coding Agents (Within Kanban Crew)

A powerful workflow involves using coding agents within Kanban Crew that are also connected to the Kanban Crew MCP server:
...
4. **Automatic Population**: The agent uses the MCP server to populate individual issues directly in the Kanban Crew interface
...
Raycast is a popular MCP client that can connect to Kanban Crew's MCP server. Here's how to configure it:
...
<Frame>
<img src="/images/vk-raycast-mcp-part-1.png" alt="Raycast MCP configuration - adding Kanban Crew server" />
</Frame>

Configure the Kanban Crew MCP server in Raycast by adding the server details.
</Tab>
...
Once configured, you'll see the Kanban Crew MCP server listed and ready to use in Raycast.
</Tab>
</Tabs>
